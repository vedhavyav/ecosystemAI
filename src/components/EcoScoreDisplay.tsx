...
export default function EcoScoreDisplay({ result, recommendations = [], showScore = true, showTimeline = true }: Props) {
  const dashArray = 283;
  const dashOffset = dashArray - (dashArray * result.ecoScore) / 100;
  ...
  const timelineData: TimelineItem[] = recommendations.slice(0, 5).map((rec, index) => {
    ...
    else if (desc.includes('flight') || desc.includes('transit') || desc.includes('driv')) Icon = Globe;
    ...
    return {
      ...
      status: rec.difficulty === 'Easy' ? 'completed' : rec.difficulty === 'Medium' ? 'in-progress' : 'pending',
    ...
    <div className="space-y-12">
      ...
      {showScore && (
        <div className="flex flex-col items-center justify-center relative py-8">
          ...
          <h2 className="text-black font-black mb-8 uppercase tracking-[0.3em] text-xs relative z-10 drop-shadow-sm">Your Eco Score</h2>
          
          <div className="relative w-56 h-56 flex items-center justify-center">
            <svg className="absolute w-full h-full transform -rotate-90 drop-shadow-[0_0_20px_rgba(255,255,255,0.4)]" viewBox="0 0 100 100">
              <circle 
                cx="50" cy="50" r="45" 
          ...
          <motion.div 
            ...
            className="mt-8 mb-4"
          >
            <span className="text-lg md:text-xl whitespace-nowrap font-black tracking-widest text-black uppercase bg-white/60 backdrop-blur-md px-5 py-1.5 rounded-full border border-white/80 shadow-sm drop-shadow-sm">{result.level}</span>
          </motion.div>

          <div className="text-center relative z-10 mt-2">
            <p className="text-slate-900 text-lg font-bold uppercase tracking-widest">
              <strong className="font-black text-4xl text-black block mb-1">{result.totalCO2eTons.toFixed(2)}</strong> Tons CO₂e / Yr
            </p>
      ...
      {showTimeline && (
        <div className="relative overflow-hidden py-12">
          ...
          <p className="text-emerald-100/70 mb-16 text-xl font-light relative z-10 text-center">Click on nodes to explore your recommended actions.</p>
